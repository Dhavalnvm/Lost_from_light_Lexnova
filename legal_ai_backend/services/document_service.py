    ...
    async def _retrieve_summary_context(self, document_id: str) -> str:
        ...
        for query in SUMMARY_QUERIES:
            try:
                ...
                # ✅ Now shows actual error type e.g. TimeoutException
                logger.warning(f"Summary query '{query[:30]}' failed: {type(e).__name__}: {e}")
                continue

        if not relevant_chunks:
            # Fallback 1 — pull all chunks from ChromaDB directly, no embedding needed
        ...
        if not relevant_chunks:
            # Fallback 2 — use full text from registry
