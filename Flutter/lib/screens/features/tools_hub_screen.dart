...
class ToolsHubScreen extends StatelessWidget {
                ...
                delegate: SliverChildListDelegate([
                  ...
                  _ToolCard(
                    ...
                    onTap: () => Navigator.push(context, MaterialPageRoute(
                        builder: (_) => const ContractComparisonScreen())),
                  ),
                  const SizedBox(height: 14),
                  // Feature B — requires existing doc
                  _ToolCard(
                    delay: 80,
                  ...
                  const SizedBox(height: 14),
                  // Feature C — requires existing doc
                  _ToolCard(
                    delay: 160,
                  ...
                  _ToolCard(
                    ...
                    onTap: () => Navigator.push(context, MaterialPageRoute(
                        builder: (_) => const VersionDiffScreen())),
                  ),
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Text(
                        ...
                        style: GoogleFonts.dmSans(color: AppColors.textSecondary, fontSize: 12, height: 1.4),
                      )),
                    ]),
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Enter Document ID', style: GoogleFonts.cormorantGaramond(
              color: AppColors.gold, fontSize: 20, fontWeight: FontWeight.w700)),
          const SizedBox(height: 6),
          Text('Copy the Document ID from the Analyzer tab after uploading.',
          ...
          SizedBox(
            ...
            child: DecoratedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text('Open', style: GoogleFonts.dmSans(
  ...
  void _navigateToTool(BuildContext context, String tool, String docId) {
    import_screens(context, tool, docId);
  }
}

void import_screens(BuildContext context, String tool, String docId) {
  // Import here to avoid circular dependencies
  if (tool == 'rewrite') {
    Navigator.push(context, MaterialPageRoute(
        builder: (_) => _RewriteNavigator(docId: docId)));
  } else if (tool == 'checklist') {
    Navigator.push(context, MaterialPageRoute(
        builder: (_) => _ChecklistNavigator(docId: docId)));
  }
}

// Lazy wrappers to avoid importing at module level
class _RewriteNavigator extends StatelessWidget {
  final String docId;
  const _RewriteNavigator({required this.docId});
  @override
  Widget build(BuildContext context) {
    // Import dynamically
    return _ClauseRewriteWrapper(docId: docId);
  }
}

class _ChecklistNavigator extends StatelessWidget {
  final String docId;
  const _ChecklistNavigator({required this.docId});
  @override
  Widget build(BuildContext context) {
    return _SmartChecklistWrapper(docId: docId);
  }
}

// These import the screens properly
class _ClauseRewriteWrapper extends StatelessWidget {
  final String docId;
  const _ClauseRewriteWrapper({required this.docId});
  @override
  Widget build(BuildContext context) {
...
class _SmartChecklistWrapper extends StatelessWidget {
  final String docId;
  const _SmartChecklistWrapper({required this.docId});
  @override
  Widget build(BuildContext context) {
    return _FeaturePlaceholder(
...
class _FeaturePlaceholder extends StatelessWidget {
  ...
  final IconData icon;
  const _FeaturePlaceholder({required this.title, required this.docId, required this.icon});
  @override
  Widget build(BuildContext context) => Scaffold(
    backgroundColor: AppColors.background,
    appBar: AppBar(title: Text(title, style: AppTextStyles.goldTitle.copyWith(fontSize: 18)),
        backgroundColor: AppColors.surface),
    body: Center(child: Text('Doc: $docId', style: const TextStyle(color: AppColors.textSecondary))),
  );
}
...
class _ToolCard extends StatelessWidget {
  ...
  const _ToolCard({
    required this.delay, required this.icon, required this.gradient,
    required this.accentColor, required this.title, required this.subtitle,
    required this.tags, this.badge, required this.onTap,
  });
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            if (badge != null)
              Container(
                ...
                child: Text(badge!, style: const TextStyle(color: AppColors.textSecondary, fontSize: 11)),
              ),
            const SizedBox(width: 8),
            Icon(Icons.arrow_forward_ios_rounded, color: accentColor.withOpacity(0.6), size: 14),
          ]),
          const SizedBox(height: 16),
          Text(title, style: GoogleFonts.cormorantGaramond(
              color: accentColor, fontSize: 22, fontWeight: FontWeight.w700, height: 1.2)),
          ...
          Wrap(spacing: 6, runSpacing: 6, children: tags.map((t) => Container(
            ...
            child: Text(t, style: TextStyle(color: accentColor, fontSize: 11, fontWeight: FontWeight.w500)),
          )).toList()),
        ]),
