...
class _ProfileScreenState extends State<ProfileScreen> {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: _loading
            ...
            : _error != null
                ? Center(child: Text(_error!, style: const TextStyle(color: AppColors.dangerRed)))
                : RefreshIndicator(
                    onRefresh: _load,
                        ...
                        if (_docs.isNotEmpty) ...[
                          SliverToBoxAdapter(
                            child: Padding(
                              ...
                              child: Text('DOCUMENT HISTORY', style: AppTextStyles.sectionTitle),
                            ),
                          ),
                          SliverPadding(
                            ...
                            child: Padding(
                              ...
                              child: Center(
                                child: Column(children: [
                                  const Icon(Icons.folder_open_rounded, color: AppColors.textMuted, size: 48),
                                  const SizedBox(height: 12),
                                  Text('No documents yet', style: GoogleFonts.dmSans(
                                      color: AppColors.textMuted, fontSize: 16)),
                                  const SizedBox(height: 6),
                                  Text('Upload a document in the Analyzer tab',
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          ...
          child: Center(
            child: Text(_profile!.name.isNotEmpty ? _profile!.name[0].toUpperCase() : 'U',
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(_profile!.email, style: const TextStyle(color: AppColors.textSecondary, fontSize: 13)),
        ])),
        IconButton(
          onPressed: _logout,
  ...
  Widget _buildStats() {
    ...
    return Padding(
      ...
      child: Row(children: [
        Expanded(child: _StatCard(label: 'Documents Analyzed', value: '${_docs.length}',
            icon: Icons.description_rounded, color: AppColors.gold)),
        const SizedBox(width: 12),
        Expanded(child: _StatCard(label: 'Member Since', value: _profile!.createdAt.isNotEmpty
            ? _profile!.createdAt.substring(0, 10) : 'Today',
            icon: Icons.calendar_today_rounded, color: AppColors.infoBlue)),
      ]),
    );
  }
}
...
class _DocHistoryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = riskLevel == 'High' ? AppColors.dangerRed
        : riskLevel == 'Medium' ? AppColors.warningAmber : AppColors.safeGreen;
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(doc.filename, style: GoogleFonts.dmSans(
              color: AppColors.textPrimary, fontWeight: FontWeight.w600, fontSize: 13),
              maxLines: 1, overflow: TextOverflow.ellipsis),
          const SizedBox(height: 2),
          Text(doc.uploadedAt.isNotEmpty ? doc.uploadedAt.substring(0, 10) : '',
              style: const TextStyle(color: AppColors.textMuted, fontSize: 11)),
        ])),
        if (safetyScore != null) ...[
          const SizedBox(width: 10),
          Column(children: [
            Text('$safetyScore', style: GoogleFonts.cormorantGaramond(
      ...
      child: Padding(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Container(
            ...
            child: const Icon(Icons.person_outline_rounded, color: AppColors.gold, size: 40),
          ),
          const SizedBox(height: 24),
          Text('Your Profile', style: AppTextStyles.goldTitle),
          const SizedBox(height: 8),
          const Text('Sign in to track your documents\nand access your analysis history',
