...
class _DocumentAnalyzerScreenState extends State<DocumentAnalyzerScreen> {
  ...
  Future<void> _analyzeDocument() async {
    ...
    try {
      ...
      // Step 2: Stream SSE — ✅ FIXED: pass data map directly, no re-encoding
      await for (final event in _api.analyzeStream(upload.documentId, _analysisMode)) {
        final eventName = event['event'] as String;
        final data = event['data'] as Map<String, dynamic>; // ✅ already decoded
        _handleSseEvent(eventName, data); // ✅ pass map directly
      }
    } catch (e) {
      if (!mounted) return;
      setState(() {
  ...
  // ✅ FIXED — accepts Map directly, no jsonDecode needed
  void _handleSseEvent(String event, Map<String, dynamic> data) {
    if (!mounted) return;

    setState(() {
      switch (event) {
        case 'status':
          _currentStep = data['message'] ?? '';
          break;

        case 'summary':
          try {
            _summary = DocumentSummary.fromJson(data);
            _completedSteps.add('summary');
            debugPrint('✅ Summary received: ${_summary?.summary.length} chars');
          } catch (e) {
            debugPrint('❌ Summary parse error: $e');
            _completedSteps.add('summary_error');
          }
          break;

        case 'risk':
          try {
            _riskAnalysis = RiskAnalysis.fromJson(data);
            _completedSteps.add('risk');
            debugPrint('✅ Risk received: ${_riskAnalysis?.detectedRedFlags.length} flags');
          } catch (e) {
            debugPrint('❌ Risk parse error: $e');
            _completedSteps.add('risk_error');
          }
          break;

        case 'fairness':
          try {
            _clauseFairness = ClauseFairness.fromJson(data);
            _completedSteps.add('fairness');
            debugPrint('✅ Fairness received: ${_clauseFairness?.clausesAnalyzed.length} clauses');
          } catch (e) {
            debugPrint('❌ Fairness parse error: $e');
            _completedSteps.add('fairness_error');
          }
          break;

        case 'safety':
          try {
            _safetyScore = SafetyScore.fromJson(data);
            _completedSteps.add('safety');
            debugPrint('✅ Safety score: ${_safetyScore?.safetyScore}');
          } catch (e) {
            debugPrint('❌ Safety parse error: $e');
            _completedSteps.add('safety_error');
          }
          break;
        ...
        case 'done':
          // ✅ FIXED — don't set _isAnalyzing=false here immediately.
          // Use addPostFrameCallback so all setState from prior events
          // are fully flushed before we hide the loading state.
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                // ✅ FIXED — show tabs as soon as ANY result arrives,
                // including while still analyzing (progressive rendering)
                if (_summary != null ||
                    _riskAnalysis != null ||
                    _clauseFairness != null) ...[
                  const SizedBox(height: 24),
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.auto_awesome_rounded,
              ...
              duration: 1500.ms, color: AppColors.goldLight),
